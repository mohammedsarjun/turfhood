...
-split Components based on features then add all components into an page then add the page to nextjs page
-Api routes should live in lib/apiRoutes
-Api call should happen in action folder
-Add Shared Dto types on shared folder
-hooks should live on hooks
-shared types should live on types
-Optimize the using React.memo, useCallback,useMemo Whereever its needed (dont use it everywhere only use when its really help to optimize performance )
-use react.memo only when the child component doing computationally expensive operations
-after a creation,updation,deletion or any other waiting operations should indicate to user via toast context 
-Test important components using jest

# Backend Tasks
- Follow Solid Principles and Oops concept
- Follow Clean Architecture
- Test use case and intergration test using mocha+chai and supertest
- Avoid writing success message and error message hardcoded instead use a constants
- If the operation expects all or nothing behaviour use mongoose transaction
- dont write any business login inside repository
...
- Don't install dependencies unless necessary.
- Don't create duplicate API clients/utilities.
