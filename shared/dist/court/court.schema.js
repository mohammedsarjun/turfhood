...
export const createAvailabilityOverrideSchema = z
    ...
    if (value.isClosed) {
        ...
        if (value.blockedSlots.length > 0)
            context.addIssue({ code: z.ZodIssueCode.custom, path: ["blockedSlots"], message: "A closed day cannot also contain blocked slots." });
        return;
    }
    if (value.closureReason)
    ...
    if (uniqueSlots.size !== value.blockedSlots.length)
        context.addIssue({ code: z.ZodIssueCode.custom, path: ["blockedSlots"], message: "A slot can only be blocked once." });
});
