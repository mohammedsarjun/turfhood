import { z } from "zod";
import { ALLOWED_SLOT_DURATIONS, SLOT_DURATION_ERROR_MESSAGE, RAILWAY_TIME_PATTERN, } from "./court.constants.js";
const timeSchema = z
    .string()
...
export const createCourtSchema = z
    ...
    slotDurationMinutes: z
        .number()
        .int()
        .refine((duration) => ALLOWED_SLOT_DURATIONS.some((allowed) => allowed === duration), SLOT_DURATION_ERROR_MESSAGE),
    imageCoverFlags: z
        .array(z.boolean())
...
const blockedPeriodSchema = availabilityPeriodSchema.and(z.object({ reason: z.string().trim().max(200, "Reason cannot exceed 200 characters.").optional() }));
function addOverlapIssues(periods, path, context) {
    periods.forEach((period, index) => {
        if (periods.some((candidate, candidateIndex) => candidateIndex !== index && period.startTime < candidate.endTime && candidate.startTime < period.endTime)) {
...
export const createAvailabilityOverrideSchema = z
    ...
    .superRefine((value, context) => {
    if (value.isClosed) {
        if (!value.closureReason)
            context.addIssue({ code: z.ZodIssueCode.custom, path: ["closureReason"], message: "Select a closure reason." });
        if (value.customHours || value.blockedPeriods.length > 0)
            context.addIssue({ code: z.ZodIssueCode.custom, path: ["isClosed"], message: "A closed day cannot contain availability periods." });
        return;
    }
    if (value.closureReason)
        context.addIssue({ code: z.ZodIssueCode.custom, path: ["closureReason"], message: "Closure reason is only allowed for a closed day." });
    if (!value.customHours && value.blockedPeriods.length === 0) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            path: ["blockedPeriods"],
...
export const updateCourtSchema = z
    .preprocess((value) => typeof value === "object" && value !== null
    ? { ...value, imageCoverFlags: [true] }
    : value, createCourtSchema)
