...
describe('OtpForm', () => {
  beforeEach(() => {
    ...
    // useCountdown persists its deadline to sessionStorage, which jsdom keeps
    // across tests in the same file — clear it so each test starts fresh.
  ...
  it('submits the full code and redirects home on success', async () => {
    ...
    render(<OtpForm email="jordan@example.com" purpose="login" />);
    await typeCodeAndSubmit('123456');

    await waitFor(() => {
      expect(verifyOtpMock).toHaveBeenCalledWith({ email: 'jordan@example.com', otp: '123456', purpose: 'login' });
    });
    await waitFor(() => {
  ...
  describe('resend countdown', () => {
    ...
    it('calls resendOtp and resets the countdown when Resend Code is clicked', async () => {
      resendOtpMock.mockResolvedValueOnce({ message: 'Verification code sent.', expiresInSeconds: 60 });
      render(<OtpForm email="jordan@example.com" purpose="login" />);
      ...
      expect(resendOtpMock).toHaveBeenCalledWith({ email: 'jordan@example.com', purpose: 'login' });
      await waitFor(() => {
        expect(screen.queryByRole('button', { name: /resend code/i })).not.toBeInTheDocument();
      });
    });
