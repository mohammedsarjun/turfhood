...
describe('ManageCourtDetailsUseCase', () => {
  it('returns the owned court with its availability overrides', async () => {
    ...
    const result = await new ManageCourtDetailsUseCase(courts, turfs).get({
  ...
  it('rejects a court outside the resolved turf', async () => {
    ...
    try {
      await new ManageCourtDetailsUseCase(courts, turfs).get({
        portalTurfId: 'application_1',
        courtId: 'other_court',
        ownerId: 'owner_1',
      });
  ...
  it('rejects an update when another court has the same name', async () => {
    ...
    try {
      await new ManageCourtDetailsUseCase(courts, turfs).update({
        ...
        slotDurationMinutes: 60,
        pricingRules: [
          { dayType: 'weekday', startTime: '06:00', endTime: '10:00', pricePerSlot: 500 },
        ],
      });
      expect.fail('Expected duplicate name to be rejected.');
    } catch (error) {
      expect(error).to.be.instanceOf(DuplicateCourtNameError);
    }
  });
});
