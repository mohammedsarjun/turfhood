...
export class OwnerTurfController {
  ...
  details = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const turf = await this.turfs.findOwnedByIdOrVerificationId(req.params.id as string, this.userId(req));
      ...
      res.status(HttpStatus.OK).json({ turf: {
        id: turf.id, name: turf.name, description: turf.description ?? '', address: turf.address,
        location: { latitude, longitude }, images: imageMap.get(turf.id) ?? [],
      }});
  ...
  update = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { name, description, address, location } = (req as ValidatedOwnerTurfRequest).validatedOwnerTurf!;
      ...
      const turf = await this.turfs.updateBasicDetails(req.params.id as string, this.userId(req), {
        ...
        address, location: { type: 'Point', coordinates: [location.longitude, location.latitude] },
      });
  ...
  cover = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const turf = await this.turfs.findOwnedByIdOrVerificationId(req.params.id as string, this.userId(req));
      if (!turf?.id) throw new AppError('Turf not found.', HttpStatus.NOT_FOUND);
      const uploaded = await this.files.upload({ buffer: file.buffer, filename: file.originalname, mimeType: file.mimetype, folder: 'turf-images', resourceType: 'image' });
  ...
  gallery = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      try { retainedUrls = JSON.parse(String(req.body.retainedUrls ?? '[]')); } catch { retainedUrls = null; }
      if (!Array.isArray(retainedUrls) || !retainedUrls.every((url) => typeof url === 'string' && currentUrls.includes(url)))
      ...
      if (total < 1 || total > 10) throw new AppError('Keep between 1 and 10 turf photos.', HttpStatus.BAD_REQUEST);
      ...
      for (const file of files) {
        const uploaded = await this.files.upload({ buffer: file.buffer, filename: file.originalname, mimeType: file.mimetype, folder: env.CLOUDINARY_TURF_IMAGE_FOLDER });
      ...
      await Promise.all(currentUrls.filter((url) => !retainedUrls.includes(url)).map((url) => this.files.delete(url)));
