...
export class OpenSessionController {
  constructor(@inject(OPEN_SESSION_TOKENS.UseCase) private readonly sessions: IManageOpenSessionsUseCase) {}

  create = async (req: Request, res: Response, next: NextFunction) => {
    try { res.status(HttpStatus.CREATED).json(await this.sessions.create(userId(req), req.body as CreateOpenSessionRequest)); } catch (error) { next(error); }
  };
  join = async (req: Request, res: Response, next: NextFunction) => {
    try { res.status(HttpStatus.CREATED).json(await this.sessions.join(userId(req), String(req.params.id))); } catch (error) { next(error); }
  };
    ...
    try {
      ...
      const location =
        latitude !== undefined && longitude !== undefined && Number.isFinite(latitude) && Number.isFinite(longitude)
          ? { latitude, longitude }
          : {};
      res.json(await this.sessions.list({ page, limit, ...(typeof req.query.sportTypeId === 'string' ? { sportTypeId: req.query.sportTypeId } : {}), ...location }));
    } catch (error) { next(error); }
  };
  details = async (req: Request, res: Response, next: NextFunction) => {
    try { res.json(await this.sessions.details(String(req.params.id))); } catch (error) { next(error); }
  };
  payuSuccess = async (req: Request, res: Response) => {
    ...
    } catch { res.redirect(`${env.FRONTEND_URL}/open-sessions?payment=failed`); }
  };
  payuFailure = async (req: Request, res: Response) => {
    try { await this.sessions.paymentCallback(req.body as PaymentCallback); } catch { /* recorded by use case */ }
    const id = typeof req.body.udf1 === 'string' ? req.body.udf1 : '';
    res.redirect(id ? `${env.FRONTEND_URL}/open-sessions/${id}?payment=failed` : `${env.FRONTEND_URL}/open-sessions?payment=failed`);
  };
