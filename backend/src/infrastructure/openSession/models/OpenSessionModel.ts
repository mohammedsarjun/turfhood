...
const schema = new Schema<OpenSessionDocument>(
  {
    ...
    status: { type: String, enum: ['awaiting_creator_payment', 'open', 'full', 'cancelled', 'completed'], required: true, index: true },
    ...
    participants: [{
      ...
      paymentId: String,
      refundRequestId: String,
      paymentStatus: { type: String, enum: ['pending', 'paid', 'refund_pending', 'refunded', 'refund_failed'], required: true },
...
export const OpenSessionModel: Model<OpenSessionDocument> =
  (mongoose.models.OpenSession as Model<OpenSessionDocument> | undefined) ?? model<OpenSessionDocument>('OpenSession', schema);
