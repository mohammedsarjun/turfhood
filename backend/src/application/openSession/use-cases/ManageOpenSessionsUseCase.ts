...
export class ManageOpenSessionsUseCase implements IManageOpenSessionsUseCase {
  ...
  async create(userId: string, input: CreateOpenSessionRequest) {
    ...
    if (!court.allowOpenSessions)
      throw new BookingActionError('This court does not allow open sessions.');
    if (!court.sportTypeIds.includes(input.sportTypeId))
      throw new BookingActionError('Select a sport supported by this court.');
    if (
      !Number.isInteger(input.maximumPlayers) ||
      input.maximumPlayers < court.minPlayersForOpenSession ||
      input.maximumPlayers > court.capacity
    )
      throw new BookingActionError(
        `Players must be between ${court.minPlayersForOpenSession} and ${court.capacity}.`,
      );
    ...
    const override = (await this.courts.listOverrides(input.courtId)).find(
      (item) => item.date === input.bookingDate,
    );
    ...
    const occupied = await this.bookings.occupiedStarts(
      input.courtId,
      [input.bookingDate],
      new Date(),
    );
    if (occupied.get(input.bookingDate)?.has(input.startTime))
      throw new BookingActionError('The selected slot is no longer available.');
    ...
    const created = await this.sessions.create({
      ...
      ...((court.images.find((image) => image.isCover)?.url ?? court.images[0]?.url)
        ? { courtImage: court.images.find((image) => image.isCover)?.url ?? court.images[0]!.url }
        : {}),
      address: [
        turf.address.line1,
        turf.address.city,
      ...
      totalPricePaise,
      pricePerParticipantPaise: perHead,
  ...
  async join(userId: string, sessionId: string) {
    ...
    const transactionId = `os-${randomUUID().replaceAll('-', '').slice(0, 27)}`;
    const session = await this.sessions.addPendingParticipant(
      sessionId,
      { id: userId, name: user.name },
      transactionId,
    );
    if (!session)
      throw new BookingActionError('This session is full, closed, or you already joined it.');
    return {
      session,
      payment: this.paymentForm(session, transactionId, user.name, user.email.toString()),
    };
  }
  ...
  async list(input: {
    ...
    const result = await this.sessions.list({
      page: input.page,
      limit: input.limit,
      ...(input.sportTypeId ? { sportTypeId: input.sportTypeId } : {}),
  ...
  async paymentCallback(input: PaymentCallback) {
    ...
    const transactionExpired = payment
      ? payment.status === 'pending' && isOpenSessionPaymentExpired(payment.createdAt, new Date())
      : false;
    if (
      !this.payments.verifyCallback(input) ||
      input.status !== 'success' ||
    ...
    if (
      (transactionExpired ||
        ...
        new Date(session.fillDeadline) <= new Date()) &&
      input.mihpayid
    ) {
      await this.payments.refund(
        input.mihpayid,
  ...
  async cancelParticipation(userId: string, sessionId: string) {
    ...
    try {
      const refund = await this.payments.refund(
        ...
        cancellation.amountPaise,
        `os-leave-${sessionId.slice(-10)}-${userId.slice(-8)}`,
      );
      await this.sessions.markParticipantRefundRequested(sessionId, userId, refund.requestId);
    } catch (error) {
      await this.sessions.markParticipantRefundResult(sessionId, userId, 'refund_failed');
      throw error;
    }
  ...
  async expireUnfilled() {
    await this.sessions.expirePendingParticipants(new Date());
  ...
  private paymentForm(
    session: { id: string; pricePerParticipantPaise: number; turfName: string; sportName: string },
    transactionId: string,
    name: string,
    email: string,
  ) {
    return this.payments.createOpenSessionForm({
      transactionId,
      sessionId: session.id,
      amountPaise: session.pricePerParticipantPaise,
      customerName: name,
      customerEmail: email,
