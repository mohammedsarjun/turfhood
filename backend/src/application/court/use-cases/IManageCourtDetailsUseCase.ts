...
export interface CourtAccessInput {
  portalTurfId: string;
  courtId: string;
  ownerId: string;
}
export interface IManageCourtDetailsUseCase {
  ...
  createOverride(
    input: CourtAccessInput & CreateAvailabilityOverrideRequest,
  ): Promise<AvailabilityOverrideDTO>;
  updateOverride(
    input: CourtAccessInput & CreateAvailabilityOverrideRequest & { overrideId: string },
  ): Promise<AvailabilityOverrideDTO>;
